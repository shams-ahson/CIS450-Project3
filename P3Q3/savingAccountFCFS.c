 ...
 *  The transaction is first come first served.
*/
...
int main(int argc, char* argv[]) {
    ...
    pthread_cond_init(&account.bank_cv, NULL);
    ...
    pthread_cond_destroy(&account.bank_cv);
...
void withdraw(Account* account, int amount, int id, int i) {
    ...
    // only allow the first thread in the waiting queue to be broadcasted
    account->nw++;
    if (account->nw > 1) {
        printf("Thread %d Transaction %d amount %d us number %d in the queue\n", id, i, amount, account->nw);
        pthread_cond_wait(&account->bank_cv, &account->mutex);
    }

    // wait if the balance is insufficient
    while(account->balance < amount) {
        printf("Thread %d Transaction %d fails to withdraw %d and is number %d in the queue\n", id, i, amount, account->nw);
    ...
    account->nw--;
    pthread_cond_signal(&account->bank_cv);
    pthread_mutex_unlock(&account->mutex);
    sched_yield();
}
